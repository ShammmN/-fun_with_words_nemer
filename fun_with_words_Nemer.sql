...
--Question 3: --
CREATE OR REPLACE VIEW WORD_REL_READ AS SELECT * FROM WORD_REL WITH READ ONLY;

UPDATE WORD_REL_READ SET SYN_COL = 'blithe' WHERE WORD = 'insouciant';
...
CREATE OR REPLACE VIEW WORD_REL (word, syn_col, ant_col) AS
SELECT s.word_term, s.syn_word, a.ant_word FROM WORD_SYNONYMS s, WORD_ANTONYMS a WHERE a.word_id = s.word_id;

--Question 7: --
SELECT * FROM WORD_SUMMARY;

--Question 8: (I recieved help) --
CREATE OR REPLACE VIEW HOMONYM_MEANINGS AS
SELECT WORD_TERM, LISTAGG(meaning, ', ') WITHIN GROUP (ORDER BY HOM_ID) AS MEANINGS FROM HOMONYMS GROUP BY WORD_TERM;
